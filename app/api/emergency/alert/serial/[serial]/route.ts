...
import { getEmergencyAlertBySerialNumber } from '@/lib/db/emergency';
import { ObjectId } from 'mongodb';

// GET - Get emergency alert details by serial number
export async function GET(
  ...
  try {
    ...
    const serialNumber = resolvedParams.serial;
    const alert = await getEmergencyAlertBySerialNumber(serialNumber);
    ...
    return NextResponse.json({
      ...
      alert: {
        ...
        donation_time_start: alert.donation_time_start,
        donation_time_end: alert.donation_time_end,
        contact_info: alert.contact_info,
        reference: alert.reference,
        status: alert.status,
        donors_notified: alert.donors_notified,
        donors_responded: alert.donors_responded,
        selected_donor_id: alert.selected_donor_id?.toString(),
    ...
    console.error('Get emergency alert by serial error:', error);
    return NextResponse.json(
